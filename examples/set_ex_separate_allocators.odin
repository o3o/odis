...
main :: proc() {
	...
	key := "odis:set-ex:separate-allocators:example"

	// The client uses the allocator captured by `connect`.
	// This arena is only for the replies returned by each command.
	mem.free_all(reply_allocator)
	set_reply, set_err := redis.set_ex(&client, key, "42", 60, reply_allocator)
	...
	fmt.println("set_ex with separate client/reply allocators passed")
}
